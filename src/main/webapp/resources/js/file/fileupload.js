var uploadModule = (function () {
  ...
  $(document).ready(function () {
    ...
    $("body").append(
      '<form name="downloadForm" id="downloadForm" method="post" action="' +
        basePath +
        '/file/download/zip" style="display:none"><input type="hidden" name="downloadFileInfo" id="downloadFileInfo" value=""><input type="hidden" name="_csrf" value="' +
        token +
        '"></form>'
    );
  });
  ...
  function getFileList(obj) {
    ...
    var thumbnailYn = $(obj).data("thumbnailYn");
    ...
    if (docId !== "") {
      $.ajax({
        ...
        data: { docId: docId, thumbnailYn: thumbnailYn },
        datatype: "json",
  ...
  var chunk_size = 1 * 1024 * 1024 * 20; // 20Mbyte Chunk, was의 chunkSize 보다 같거나 작게 잡아야 함
  ...
  function upload(uploadFormId) {
    if ($("#fileInfoList_" + uploadFormId).val() != "") {
      ...
      if (maxFileCnt != "") {
        if (tmpFileList.length >= maxFileCnt) {
  ...
  function setUploadInfo(fileList, uploadFormId) {
    if (fileList) {
      ...
      metadata.thumbnailYn = thumbnailYn;
  ...
  function uploadFile(piece, metadata) {
    ...
    //var isThumbnail = $("#isThumbnail_"+metadata.uploadFormId).val();
    ...
    $.ajax({
      ...
      success: function (result) {
        if (result.hasOwnProperty("errorCode")) {
          $("#progressLabel_" + metadata.uploadFormId).html(
            result.errorMessage
          );
        ...
        if (fileInfo.uploadComplete != "Y") {
          ...
          $("#progressBar_" + metadata.uploadFormId).attr(
            "data-label",
            number + "%"
          );
  ...
  function displayFileList(fileInfo, uploadFormId) {
    ...
    if (fileInfo.delYn == "N") {
      ...
      fileObj.thumbnailYn = thumbnailYn;
      ...
      var str =
        "<a href=\"javascript:uploadModule.fileDownload('" +
        uploadFormId +
        "', '" +
        fileInfo.fileId +
        "', '" +
        ...
        fileNm +
        "</a> [" +
        Math.round(Number(fileInfo.fileSize / 1024)) +
        "k]";

      $("#fileList_" + uploadFormId).append(
        "<div data-file-Id='" +
          fileInfo.fileId +
          "' style='padding-left:8px'><input type='checkbox' " +
          checked +
          " name='file_" +
          uploadFormId +
          "' value='" +
      ...
      $("#fileList_" + uploadFormId + " > div").each(function () {
        if (
          $(this).data("fileId") == fileInfo.fileId &&
          fileInfo.delYn == "Y"
        ) {
          $(this).remove();
    ...
    if (metadata.totalChunks >= metadata.chunkIndex) {
      ...
      $("#progressLabel_" + metadata.uploadFormId).html(
        "Uploading: [파일명 :" +
          metadata.fileNm +
          "]" +
          ((100 * metadata.start) / file.size).toFixed(0) +
          "%"
      );
  ...
  function getFileArray(uploadFormId, files) {
    ...
    for (var i = 0; i < files.length; i++) {
      ...
      if (acceptFile.toUpperCase().indexOf(fileExt.toUpperCase()) == -1) {
        $("#progressLabel_" + uploadFormId).html(
          "파일확장자가 " + fileExt + "인 파일은 업로드 할수 없습니다."
        );
        continue;
      }
      ...
      if (isNotEmpty(maxFileSize)) {
        ...
        if (files[i].size > tmpmaxFileSize * 1024 * 1024) {
          $("#progressLabel_" + uploadFormId).html(
            "파일사이즈가 " + maxFileSize + "이하만 업로드 가능합니다."
          );
          continue;
        }
      } else {
  ...
  function fnFileImg(fileNm) {
    ...
    if (retFlag) {
      retStr =
        '<img src="' +
        basePath +
        "/images/filemanager/attach_" +
        name +
        '.gif">';
    } else {
      retStr = '<img src="' + basePath + '/images/filemanager/icon_doc.gif">';
    }
    return retStr;
  }
