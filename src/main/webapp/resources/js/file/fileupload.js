...
var uploadModule =(function() {
	...
	//업로드 정보세팅
		  ...
		  $.ajax({
			    ...
			    success: function (result) {
			    	...
			    	if(fileInfo.uploadComplete != 'Y'){
				    	  ...
				    	  $("#progressBar_"+metadata.uploadFormId).attr('data-label',number+'%');
			    		...
			    		$("#progressLabel_"+metadata.uploadFormId).html("upload completed: " +"100%" );
			    		$("#file_input_"+metadata.uploadFormId).attr('disabled', false);
		 ...
		 if(fileInfo.delYn == 'N'){
			...
			*/
		         ...
		         if(fileInfo.selected){
		          ...
		          var str='<a href="javascript:uploadModule.fileDownload(\''+uploadFormId+'\', \''+fileInfo.fileId+'\', \''+fileInfo.fileNm+'\',\''+fileInfo.temp+'\');">'+fileNm+'</a> ['+Math.round(Number(fileInfo.fileSize/1024))+'k]';
	 ...
	 function slice(metadata) {
		 ...
		 if(metadata.totalChunks >= metadata.chunkIndex){
			 ...
			 $("#progressLabel_"+metadata.uploadFormId).html("Uploading: [파일명 :" +  metadata.fileNm +"]"+ (100*metadata.start/file.size).toFixed(0) + "%" );
    ...
    function fnFileImg(fileNm){
	     ...
	     var retStr = '';
	     if(retFlag){
	    	 retStr = '<img src="'+basePath+'/images/filemanager/attach_'+name+'.gif">';
	     }else{
